...
const getCropRecommendation = async (req, res) => {
  try {
    ...
    const prompt = `As an agricultural expert, provide crop recommendations for a farmer with the following details:
...
Format the response in a clear, structured manner suitable for farmers.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert agricultural consultant with deep knowledge of farming practices, crop selection, and market analysis. Provide practical, actionable advice for farmers."
        },
        {
          role: "user",
          content: prompt
...
const getPlantingAdvice = async (req, res) => {
  try {
    ...
    // Create prompt for AI
    const prompt = `As an agricultural expert, provide detailed planting advice for ${crop} cultivation with the following details:
...
Format the response in a clear, step-by-step manner suitable for farmers.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert agricultural consultant specializing in crop cultivation and best practices. Provide detailed, practical planting advice that farmers can implement immediately."
        },
        {
          role: "user",
          content: prompt
...
const getPestDiagnosis = async (req, res) => {
  try {
    ...
    const prompt = `As an agricultural expert, help diagnose potential pest or disease issues for ${crop} with the following symptoms:
...
Format the response in a clear, actionable manner.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert agricultural consultant specializing in pest and disease management. Provide accurate diagnosis and treatment recommendations."
        },
        {
          role: "user",
          content: prompt
